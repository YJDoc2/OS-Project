...
int syscall_create_thread(void *loc,void *arg,int pri){
	return syscall(SYSCALL_CREATE_THREAD,(uint32_t)loc,(uint32_t)arg,(uint32_t)pri,0,0);
}

int syscall_process_join(){
	return syscall(SYSCALL_PROCESS_JOIN,0,0,0,0,0);
}

char syscall_getchar_nonblock(){
	return syscall(SYSCALL_GETCHAR_NBLK,0,0,0,0,0);
}
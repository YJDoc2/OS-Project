...
static void process_switch(int newstate)
{
	...
	if(current) {
		...
		if(newstate == PROCESS_STATE_READY) {
			list_p_push(&ready_list, &current->node);
		}
	...
	while(1) {
		current = (struct process *) list_p_pop(&ready_list);
		if(current)
			break;
	...
	// if(current->node.priority -1 > MAX_PEN)
...
int thread_try_wakeup_parent(int parent_thread_id){
	if(parent_thread_id>PROCESS_MAX_PID) return -1;
	struct process *p = process_table[parent_thread_id];
	--(p->num_t);
	if(p->num_t ==0 && p->join_stat){
		list_p_push(&ready_list,&p->node);
	}
	return 0;
}

...
int sys_process_join(){
	return process_join();
}

void thread_cleanup(){
	thread_try_wakeup_parent(current->ppid);
}

int thread_run(void){
	// Ideally process coming here is a thread
	if(current->t_start == interrupt_stack_pointer){
		return -1;
	}
	((void(*)(void *))current->t_start)(current->t_args);
	thread_cleanup();
	process_exit(0);
}

int sys_create_thread(void *loc,void *arg){
	...
	process_inherit(current, p);
	p->pagetable = current->pagetable;
	
	process_stack_reset(p, PAGE_SIZE);
	
	process_kstack_reset(p,loc);
	struct x86_stack *s = (struct x86_stack *) p->kstack_ptr;
	p->t_start = loc;
	p->t_args = arg;
	s->old_eip = (unsigned) thread_run;
	
	process_launch(p);
	
	
	return current->num_t;
...
char sys_getchar_nonblock(){
	return keyboard_read(1);
}
...
void init_penalty(){
	...
	for(int i=SYSCALL_PROCESS_YIELD;i<=SYSCALL_PROCESS_PARENT;++i){
		penelty[i] = 5;
	}
	penelty[SYSCALL_PROCESS_KILL] = penelty[SYSCALL_PROCESS_REAP] = 10;
	for(int i=SYSCALL_PROCESS_WAIT;i<SYSCALL_PROCESS_STATS;++i){
		penelty[i] = 15;
	}
	penelty[SYSCALL_PROCESS_STATS] = 10;
	penelty[SYSCALL_PROCESS_HEAP] = 10;
	for(int i=SYSCALL_OPEN_FILE;i<SYSCALL_OPEN_PIPE;++i){
		penelty[i] = 20;
	}
	penelty[SYSCALL_OBJECT_TYPE] = 10;
	penelty[SYSCALL_OBJECT_DUP] = 25;
	for(int i=SYSCALL_OBJECT_READ;i<=SYSCALL_OBJECT_CLOSE;++i){
		penelty[i] = 50;
	}
	for(int i=SYSCALL_OBJECT_SET_TAG;i<=SYSCALL_OBJECT_MAX;++i){
		penelty[i] = 25;
	}
	penelty[SYSCALL_OBJECT_COPY] = 50;
	for(int i=SYSCALL_SYSTEM_STATS;i<=SYSCALL_DEVICE_DRIVER_STATS;++i){
		penelty[i] = 20;
	}
	penelty[SYSCALL_CHDIR] = 15;
	penelty[SYSCALL_CREATE_THREAD] = 25;
	penelty[SYSCALL_PROCESS_JOIN] = 30;
	penelty[SYSCALL_GETCHAR_NBLK] = 50;
...
{	
	...
	if((n < MAX_SYSCALL) && current) {
		...
		// if((current->node.priority)-penelty[n]>=-32000){
		// 	current->node.priority -= penelty[n];
		// }else{
		// 	current->node.priority = -32000;
		// }
	}
	switch (n) {
	case SYSCALL_DEBUG:
	...
	case SYSCALL_CREATE_THREAD:
		return sys_create_thread((void *)a,(void *)b);
	case SYSCALL_PROCESS_JOIN:
		return sys_process_join();
	case SYSCALL_GETCHAR_NBLK:
		return sys_getchar_nonblock();
	default:
